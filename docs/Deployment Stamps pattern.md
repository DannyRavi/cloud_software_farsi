...
اگر کاربر به کالیفرنیا سفر کند و سپس به سیستم دسترسی پیدا کند، احتمالاً ارتباط آنها از طریق منطقه West US 2 انجام می‌شود زیرا در هنگام درخواست(request)، این منطقه از نظر جغرافیایی نزدیکترین مکان است. با این حال، درخواست در نهایت باید با stamp 3 ارائه شود، زیرا داده‌‌های آنها در آنجا ذخیره می‌شود. سیستم مسیریابی ترافیک، اطمینان حاصل می‌کند که درخواست به stamp صحیح هدایت می‌شود.
