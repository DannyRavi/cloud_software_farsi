پردازش مربوط به سرویس backend را از یک میزبان مربوط به frontend جدا کنید، جایی که پردازش backend باید ناهمزمان یا asynchronous باشد، اما frontend  به response واضح نیاز دارد.

### **طرح صورت مسئله:**

در توسعه اپلیکیشن‌های مدرن، طبیعی است که برنامه‌های کلاینت - اغلب کدهایی که در یک سرویس  web-client (browser) اجرا می‌شوند - به APIهای remote برای ارائه business logic و ساختارهای  functionality وابسته باشند. این APIها ممکن است مستقیماً با برنامه مرتبط باشند یا ممکن است ارائه سرویس‌ها توسط  third party ها باشند. معمولاً این فراخوانی‌های API روی پروتکل HTTP(S) انجام می‌شوند و از ساختار REST پیروی می‌کنند.
