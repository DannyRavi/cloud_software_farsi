پردازش backend را از یک host فرانت اند جدا کنید، جایی که پردازش backend باید ناهمزمان و asynchronous باشد، اما فرانت اند هنوز به response واضح نیاز دارد.

## Context and problem

در توسعه برنامه‌های کاربردی مدرن، طبیعی است که برنامه‌های کلاینت - اغلب کدهایی که در یک سرویس  web-client (browser) اجرا می‌شوند - به APIهای remote برای ارائه business logic و ساختارهای  functionality وابسته باشند. این APIها ممکن است مستقیماً با برنامه مرتبط باشند یا ممکن است services provided ارائه شده توسط  third party ها باشند. معمولاً این فراخوانی‌های API روی پروتکل HTTP(S) انجام می‌شوند و از ساختار REST پیروی می‌کنند.
...
- زمان پردازش queue length 
- زمان پردازش request توسط Backend

هر یک از این عوامل می تواند تأخیر(latency) را به response اضافه کند. برخی از آنها را می توان scaling out کردن backend کاهش داد. بقیه چیزها مانند زیرساخت شبکه، تا حد زیادی خارج از کنترل توسعه دهنده برنامه هستند. اکثر APIها می توانند به اندازه کافی سریع پاسخ دهند برای برگشتن پاسخ ها از طریق یک  connection یکسان؟؟؟ .همین طور کد برنامه می‌تواند یک تماس API همزمان (synchronous API call) را به روش non-blocking برقرار کند و ظاهر پردازش ناهمزمان(asynchronous processing) را ارائه دهد که برای عملیات I/O-bound توصیه می‌شود. 
...
برخی از معماری ها با استفاده از یک message broker برای جداسازی مراحل request و response این مشکل را حل می کنند. این جداسازی اغلب با استفاده از [Queue-Based Load Leveling pattern](https://learn.microsoft.com/en-us/azure/architecture/patterns/queue-based-load-leveling) حاصل می شود. این جداسازی می تواند به client process و backend API  اجازه دهد تا به طور مستقل scale شوند. اما این جداسازی همچنین پیچیدگی بیشتری را در زمانی که client به success notification نیاز دارد، به همراه دارد، زیرا این مرحله باید ناهمزمان (asynchronous) شود.
...
یکی از راه حل های این مشکل استفاده از HTTP polling است. polling برای کد client-side بسیار مفید است، زیرا ارائه call-back endpoints یا استفاده از connection در حال اجرای طولانی مدت(long running process) می تواند دشوار باشد. حتی زمانی که  callbackها مقدور هستند، کتابخانه‌ها و سرویس‌های اضافی مورد نیاز گاهی اوقات می‌توانند پیچیدگی بیشتری را اضافه کنند.

- یک  client application  درواقع یک فراخوانی همزمان (synchronous call) با API برقرار می کند و یک عملیات اجرایی طولانی مدت (long running process) در backend را راه اندازی می کند.
...
در واقع این  API باید هر دو موارد  request و  اقدامی را که باید قبل از شروع فرآیند طولانی پردازش شود را تأیید کند. اگر request نامعتبر است، بلافاصله با یک کد خطا مانند HTTP 400 (Bad Request) پاسخ دهید.
```


- پاسخ یا response دارای یک مرجع مکانی (location reference) است که به یک نقطه پایانی اشاره می کند که client می تواند برای بررسی نتیجه عملیات طولانی مدت (long running operation) روی آن رای گیری و نتیجه کند.  
...
نمودار زیر یک حالت معمولی از \ردازش مورد نظر را نشان می دهد:
...
* ممکن است لازم باشد از یک processing proxy یا [Facade](https://en.wikipedia.org/wiki/Facade_pattern "Facade pattern") (نما) برای دستکاری response headerها یا payload ها با توجه به سرویس‌های مورد استفاده شده را به کار ببرید.
...
- کدهای سمت کلاینت‌ (Client-side)، مانند برنامه های مورد استفاده در مرورگر، که در آن ارائه call-back endpoint ها برگشتی دشوار است، یا استفاده از اتصالات طولانی مدت(long-running connections)، پیچیدگی بیشتری را اضافه می کند.  
  
- فراخوانی های سرویس (Service calls) که فقط در پروتکل HTTP در دسترس است و سرویس برگشتی نمی‌تواند به دلیل محدودیت‌های firewall در سمت کلاینت، تماس‌های برگشتی را برقرار کند.  
...
- client باید نتایج زیادی را جمع آوری کند و تأخیر دریافتی آن نتایج مهم است. به جای آن یک الگویservice bus را در نظر بگیرید.  
