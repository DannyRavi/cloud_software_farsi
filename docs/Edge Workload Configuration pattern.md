...
* نقش ها و وظایفی  را که محیط ابری و ذخیره‌گاه داده در  لبه (edge datastores) در پیوستگی مسائل تجاری برنامه بازی می کنند را به خوبی درک کنید. اگر اگر ذخیره ساز ابری داده‌ها  تنها منبع  موجود باشد، باید  workload لبه با استفاده از فرآیندهای خودکار بتواند حالت های مورد نظر را بازیابی کند.  
...
با لبه در اینترنت اشیاء (IoT Edge)، کنترل‌کننده پیکربندی لبه را می‌توان به‌عنوان یک ماژول پیاده‌سازی کرد و تنظیمات را می‌توان به ماژول دوقلو ([module twins](https://learn.microsoft.com/en-us/azure/iot-hub/iot-hub-devguide-module-twins)) اعمال کرد. این ماژول دوقلو دارای محدودیت در اندازه خود است. اگر پیکربندی از حد مجاز فراتر رفت، راه حل را می توان با [extended with Azure Blob Storage](https://github.com/Azure-Samples/azure-iot-hub-large-twin-example) یا با تقسیم بارهای بزرگتر از طریق روش های مستقیم 
...
# تا اینجااااااااااااا
مزایای این تغییرات عبارتند از:

* به غیر از ذخیره گاه داده‌ها، راه‌حل به اجزای سازنده نیاز ندارد و پیچیدگی را کاهش می‌دهد.  
