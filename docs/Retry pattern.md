...
برنامه‌ای که با عناصر در حال اجرا در محیط ابری ارتباط برقرار می‌‌کند باید نسبت به خطا‌های گذرا که می‌‌تواند در این محیط رخ دهد حساس باشد. خطاها شامل از دست دادن لحظه ‌ای اتصال شبکه و سرویس‌ها یا در دسترس نبودن موقت یک سرویس یا دچار timeout شدن آن است که زمانی که یک سرویس شلوغ (busy) است رخ می‌ دهد.
...
در فضای ابری، خطا‌های گذرا غیر معمول نیستند و باید برنامه‌ به گونه‌ای طراحی شود تا به زیبایی و شفافیت آن‌ها را مدیریت کند. این کار تأثیراتی را که خطاها می‌ توانند بر business tasks که برنامه انجام می‌ دهد را به حداقل می‌رساند.
...
*‏  **Retry after delay**: اگر خطا ناشی از یکی از  اتصالات ساده یا busy failures باشد، شبکه یا سرویس ممکن است به مدت کوتاهی نیاز داشته باشد تا مشکلات شبکه اصلاح شود یا کار‌های عقب افتاده پاک شود. application قبل از retry request باید برای زمان مناسب منتظر بماند.
...
یک خط مشی  retry  تهاجمی‌ با حداقل تأخیر بین retry‌ها و تعداد زیادی از retryها، می‌‌تواند سرویس busy را که نزدیک به ظرفیت ن‌هایی اش کار می‌‌کند، تضعیف کند. این retry policy همچنین می‌ تواند بر پاسخگویی برنامه تأثیر بگذارد به خصوص زمانی که به طور مداوم در حالت retry برای انجام عملیات ناموفق باشد.
...
*‏ برای رسیدگی به خرابی‌‌هایی که به دلیل خطا‌های گذرا نیستند، مانند exception‌های داخلی ناشی از خطا‌های منطق تجاری و پیاده سازی یک برنامه.  

*‏ به عنوان جایگزینی برای پرداختن به مسائل مقیاس پذیری در یک سیستم. اگر برنامه‌ای با خطا‌های busy به صورت مکرر مواجه می‌‌شود، اغلب نشانه آن است که سرویس یا منبعی که به آن دسترسی دارید باید بزرگ‌تر شود.
