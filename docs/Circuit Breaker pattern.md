...
**همزمانی - Concurrency**. هر قطع کننده مدار می‌تواند توسط تعداد زیادی از نمونه(instances)های همزمان یک برنامه قابل دسترسی باشد. پیاده سازی نباید requestهای همزمان را مسدود کند یا به هر فراخوانی به یک عملیات سربار اضافی وارد کند.
