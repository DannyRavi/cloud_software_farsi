...
اکثر پیاده سازیهای message queueها از چندین مصرف کننده(consumer) پشتیبانی می‌کنند. (به الگوی مصرف کنندگان رقیب  [Competing Consumers pattern](./Competing%20Consumers%20pattern.md)  مراجعه کنید.) تعداد فرآیندهای مصرف کننده را می‌توان بر اساس تقاضا کم و زیاد کرد.
```
