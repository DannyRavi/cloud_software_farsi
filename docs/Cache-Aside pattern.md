داده ها را در صورت نیاز  در حافظه cache از طریق یک  ذخیره‌گاه داده (data store) بارگیری کنید. با این کار می توان کارایی سیستم را بهبود بخشید و  به حفظ ثبات (consistency) بین داده های ذخیره شده در حافظه پنهان (cache) و داده های موجود در سایر ذخیره‌گاه داده‌ها کمک کنید.
...
یک application  می‌تواند عملکرد ذخیره‌سازی خواندنی (read-through) را با پیاده‌سازی استراتژی Cache-Aside پیاده سازی کند. این استراتژی، داده ها را در صورت لزوم در حافظه cache بارگذاری می کند.
...
**ثبات (Consistency):** پیاده سازی الگوی Cache-Aside ثبات و هماهنگی بین data store و cache را تضمین نمی کند. یک آیتم در  data store  را می‌تواند در هر زمانی با یک فرآیند خارجی تغییر کند و این تغییر ممکن است تا دفعه بعدی که آیتم مورد نظر بارگیری شود در حافظه cache منعکس نشود. در سیستمی که replicates data (@@@ این مورد به معنی تکرار پایگاه داده شناخته می شود، روشی برای کپی کردن داده ها است تا اطمینان حاصل شود که همه اطلاعات در زمان واقعی بین همه منابع داده یکسان می مانند.)  در  data storeها وجود دارد. اگر همگام‌سازی مکرر و چند مرتبه اتفاق بیفتد این مشکل می‌تواند بحرانی شود.  
...
- [Data Consistency Primer](https://learn.microsoft.com/en-us/previous-versions/msp-n-p/dn589800(v=pandp.10)).  اپلیکیشن‌های مبتنی بر محیط ابری معمولاً از داده‌هایی استفاده می‌کنند که در  data storeها پخش شده‌اند. مدیریت و حفظ ثبات داده ها در این محیط یک جنبه حیاتی از سیستم است به ویژه مسائل همزمانی (concurrency)  و در دسترس بودن (availability) که ممکن است ایجاد شود. این آغازگر مسائل مربوط به سازگاری (consistency) در میان داده های توزیع شده را توصیف و خلاصه می کند که چگونه یک اپلیکیشن می تواند سازگاری شرطی(eventual consistency) را برای حفظ در دسترس بودن داده ها پیاده سازی کند.